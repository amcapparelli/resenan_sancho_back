...
const newBookTemplate = (authorEmail, authorName, bookTitle) => {
  const from = 'resenansancho-no-reply@resenansancho.com';
  const to = authorEmail;
  const subject = '¡Enhorabuena, estás a un paso de promocionar tu libro!';
  const html = `;
  <p>Hola ${authorName},</p>
  <p>¡Ya has dado el primer paso para promocionar tu libro ${bookTitle}!</p>
  <p>Esto es lo que puedes hacer ahora para acelerar la promoción de tu libro: </p>
  ...
  <p>Empieza utilizando la opción PROMOCIONAR y selecciona la oferta de <b>ejemplares gratuitos</b> para probar el servicio.</p>
