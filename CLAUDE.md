...
- **Instagram autopost** (`lib/instagram/`) — when a book gets copies (and becomes orderable), `triggerInstagramPostIfEligible(book)` (`lib/instagram/trigger.js`) is fired **without await** from every route that adds copies (`routes/promotions.js`, `routes/paymentCheckout.js`), right after the update is persisted. **Add the same call to any new copy-adding flow.** The helper checks `copies > 0` and delegates to `publishToInstagram(book)`, whose `instagramPostedAt` guard makes repeat calls no-ops. It renders a branded JPEG with `@napi-rs/canvas`, uploads it to Cloudinary and posts it through the Meta Graph API. Every env var (`SOCIAL_AUTOPOST_ENABLED`, `IG_DRY_RUN`, …) is read at call time, never at boot. Failures are logged and swallowed — they must never affect the response. See `README.md`, `docs/instagram-autopost-spec.md` and `docs/instagram-autopost-trigger-update-spec.md`.
